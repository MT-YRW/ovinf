#include "ovinf/ovinf_humanoid_TCAttn.h"

namespace ovinf {

HumanoidTCAPolicy::~HumanoidTCAPolicy() {
  exiting_.store(true);
...
HumanoidTCAPolicy::HumanoidTCAPolicy(const YAML::Node &config) : BasePolicy(config) {
  // Read config
  ...
  Attn_size_ = config["attention_score_size"].as<size_t>();
  ...
  worker_thread_ = std::thread(&HumanoidTCAPolicy::WorkerThread, this);
}

bool HumanoidTCAPolicy::WarmUp(RobotObservation<float> const &obs_pack) {
...
bool HumanoidTCAPolicy::InferUnsync(RobotObservation<float> const &obs_pack) {
...
std::optional<HumanoidTCAPolicy::VectorT> HumanoidTCAPolicy::GetResult(
...
void HumanoidTCAPolicy::PrintInfo() {
...
void HumanoidTCAPolicy::WorkerThread() {
  if (realtime_) {
...
void HumanoidTCAPolicy::CreateLog(YAML::Node const &config) {
  ...
  // for attention score
  std::string logger_file_attn = config_file_path.string() + "/" + current_time +
                            "_atten_score_" + log_name_ + ".csv";
  ...
  for (int i = 0; i < Attn_size_; ++i) {
    attn_headers.push_back("score" + std::to_string(i));
  }

  csv_logger_ = std::make_shared<CsvLogger>(logger_file, headers);
  attn_logger_ = std::make_shared<CsvLogger>(logger_file_attn, attn_headers);
}

void HumanoidTCAPolicy::WriteLog(RobotObservation<float> const &obs_pack) {
  std::vector<CsvLogger::Number> datas;
  std::vector<CsvLogger::Number> attn_score;
  ...
  // for attention score
  for (size_t i = 0; i < Attn_size_; ++i) {
    attn_score.push_back(latest_Attn_(i));
  }

  attn_logger_ ->Write(attn_score);
}
