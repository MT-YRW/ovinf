#include "ovinf/ovinf_humanoid_Ins.h"

namespace ovinf {

HumanoidInsPolicy::~HumanoidInsPolicy() {
  exiting_.store(true);
...
HumanoidInsPolicy::HumanoidInsPolicy(const YAML::Node &config) : BasePolicy(config) {
  // Read config
  ...
  worker_thread_ = std::thread(&HumanoidInsPolicy::WorkerThread, this);
}

bool HumanoidInsPolicy::WarmUp(RobotObservation<float> const &obs_pack) {
...
bool HumanoidInsPolicy::InferUnsync(RobotObservation<float> const &obs_pack) {
...
std::optional<HumanoidInsPolicy::VectorT> HumanoidInsPolicy::GetResult(
...
void HumanoidInsPolicy::PrintInfo() {
...
void HumanoidInsPolicy::WorkerThread() {
  if (realtime_) {
...
void HumanoidInsPolicy::CreateLog(YAML::Node const &config) {
...
void HumanoidInsPolicy::WriteLog(RobotObservation<float> const &obs_pack) {
